...
class User extends CI_Controller {
 ...
 public function modal_Asesor() {
        if($this->input->post('id')){
			$id=$this->input->post('id');
            $data['Data'] = $this->User_model->get_data_by('users',$id,'users_id'); 
            $data['asesor'] = $this->User_model->get_data_by('users','Asesor','user_type'); 
            $data['id'] = $id;
			
            echo $this->load->view('add_asesor', $data, true);
        }
        exit;
    }
	public function update_asesor() {
        ...
        if($id>0) {
			$id1=$this->input->post('asesor1');
			$id2=$this->input->post('asesor2');
			if($id1!=''&&$id2!=''){
			$ass1=$this->User_model->get_data_by('users',$id1,'users_id'); 
			$ass2=$this->User_model->get_data_by('users',$id2,'users_id'); 
			$update['noass1']=$ass1[0]->users_id;
			$update['nmass1']=$ass1[0]->name;
			$update['noass2']=$ass2[0]->users_id;
			$update['nmass2']=$ass2[0]->name;
			
			$this->User_model->updateRow('users', 'users_id', $id, $update);	

            $this->session->set_flashdata('messagePr', 'Asesor Berhasil ditambahkan..');
            redirect(base_url().'user/userTable/', 'refresh'); 
			}else{
				$this->session->set_flashdata('messagePr', 'Asesor Tidak dapat ditambahkan');
				redirect(base_url().'user/userTable/', 'refresh'); 
			}
        } else { 
           
            $this->session->set_flashdata('messagePr', 'User tidak ditemukan');
            redirect(base_url().'user/userTable/', 'refresh'); 
        }
    ...
    public function dataTable (){
        ...
        foreach ($output_arr['data'] as $key => $value) {
			...
			if($users[0]->user_type=='Dosen'){
            $output_arr['data'][$key][count($output_arr['data'][$key])  - 1] .= '<button type="button" class="btn-sm  btn btn-success modalButtonAsesor" data-toggle="modal" data-src="'.$id.'"  data-target="#nameModal_Asesor"><i class="glyphicon glyphicon-user"></i>Asesor</button>    ';
			}
    ...
    public function profile($id='') {   
        ...
        $data['asesor_data'] = $this->User_model->get_data_by('users','Asesor', 'user_type');
        $this->load->view('include/header'); 
        ...
        if (!$pass)
                {
					 ...
					 $this->session->set_flashdata('messagePr', 'Format Email Tidak Dikenali');
					redirect( base_url().'user/profile', 'refresh');
                }
                else
                {
						...
						if($this->upload->do_upload('profile_pic')){
							$file = './upload/'.$_FILES['profile_pic']['name'];
							$namafile=substr($_FILES['profile_pic']['name'],0,strlen($_FILES['profile_pic']['name'])-4);
							$ext=substr($_FILES['profile_pic']['name'],strlen($_FILES['profile_pic']['name'])-4);
							$fileext=substr($_FILES['profile_pic']['name'],strlen($_FILES['profile_pic']['name'])-3);
							if(in_array($fileext,$img)){
							...
							$filename=$namafile.'_'.rand(10,1000000).$ext;
							@copy($file,'upload/profil/'.$filename);
							$profile_pic=$filename;
							}
							@unlink($file);
						}
						 if($this->upload->do_upload('sertifikat')){
							$file = './upload/'.$_FILES['sertifikat']['name'];
							$namafile=substr($_FILES['sertifikat']['name'],0,strlen($_FILES['sertifikat']['name'])-4);
							$ext=substr($_FILES['sertifikat']['name'],strlen($_FILES['sertifikat']['name'])-4);
							$filename=$namafile.'_'.rand(10,1000000000000).$ext;
							@copy($file,'upload/'.$filename);
							@unlink($file);
							$sertifikat=$filename;
						}
						if($this->upload->do_upload('ijazahs1')){
							$file = './upload/'.$_FILES['ijazahs1']['name'];
							$namafile=substr($_FILES['ijazahs1']['name'],0,strlen($_FILES['ijazahs1']['name'])-4);
							$ext=substr($_FILES['ijazahs1']['name'],strlen($_FILES['ijazahs1']['name'])-4);
							$filename=$namafile.'_'.rand(10,1000000000000).$ext;
							@copy($file,'upload/'.$filename);
							@unlink($file);
							$ijazahs1=$filename;
						...
						if($this->upload->do_upload('ijazahs2')){
							$file = './upload/'.$_FILES['ijazahs2']['name'];
							$namafile=substr($_FILES['ijazahs2']['name'],0,strlen($_FILES['ijazahs2']['name'])-4);
							$ext=substr($_FILES['ijazahs2']['name'],strlen($_FILES['ijazahs2']['name'])-4);
							$filename=$namafile.'_'.rand(10,1000000000000).$ext;
							@copy($file,'upload/'.$filename);
							@unlink($file);
							$ijazahs2=$filename;
						...
						if($this->upload->do_upload('ijazahs3')){
							$file = './upload/'.$_FILES['ijazahs3']['name'];
							$namafile=substr($_FILES['ijazahs3']['name'],0,strlen($_FILES['ijazahs3']['name'])-4);
							$ext=substr($_FILES['ijazahs3']['name'],strlen($_FILES['ijazahs3']['name'])-4);
							$filename=$namafile.'_'.rand(10,1000000000000).$ext;
							@copy($file,'upload/'.$filename);
							@unlink($file);
							$ijazahs3=$filename;
						...
						if($this->upload->do_upload('KTP')){
							$file = './upload/'.$_FILES['KTP']['name'];
							$namafile=substr($_FILES['KTP']['name'],0,strlen($_FILES['KTP']['name'])-4);
							$ext=substr($_FILES['KTP']['name'],strlen($_FILES['KTP']['name'])-4);
							$filename=$namafile.'_'.rand(10,1000000000000).$ext;
							@copy($file,'upload/'.$filename);
							@unlink($file);
							$KTP=$filename;
						 ...
						 if($id != '') {
							...
							if(isset($sertifikat)) $data['sertifikat']=$sertifikat;
						   if(isset($ijazahs1))$data['izajahs1']=$ijazahs1;
						   if(isset($ijazahs2))$data['ijazahs2']=$ijazahs2;
						   if(isset($ijazahs3))$data['ijazahs3']=$ijazahs3;
						   if(isset($KTP))$data['ktp']=$KTP;
						   
							$ass1=$this->input->post('noass1');
							$ass2=$this->input->post('noass2');
							if($ass1>0){
								$dtass1=$this->User_model->get_data_by('users',$ass1,'users_id'); 
								$data['nmass1']=$dtass1[0]->name;
							}elseif($ass1==0){
								$data['noass1']='';
								$data['nmass1']='';
							}
						   if($ass2>0){
								$dtass2=$this->User_model->get_data_by('users',$ass1,'users_id'); 
								$data['nmass2']=$dtass2[0]->name;
							}elseif($ass2==0){
								$data['noass2']='';
								$data['nmass2']='';
							}
							if(strlen($profile_pic)>0)$data['profile_pic'] = $profile_pic;
							...
							if($this->input->post('user_type') != 'admin') {
							   ...
							   if(isset($KTP))$data['ktp']=$KTP;
								...
								$ass1=$this->input->post('asesor1');
								$ass2=$this->input->post('asesor2');
								if($ass1>0){
									$dtass1=$this->User_model->get_data_by('users',$ass1,'users_id'); 
									$data['noass1']=$dtass1[0]->users_id;
									$data['nmass1']=$dtass1[0]->name;
								}elseif($ass1==0){
									$data['noass1']='';
									$data['nmass1']='';
