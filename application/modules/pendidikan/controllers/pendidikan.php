...
class pendidikan extends CI_Controller {
    ...
    function __construct() {
        ...
        //$this->pendidikan_id = isset($this->session->get_pendidikandata()['pendidikan_details'][0]->id)?$this->session->get_pendidikandata()['pendidikan_details'][0]->no:'1';
    }
    ...
    public function index() {
            ...
            $this->load->view('pendidikan_table');                
    ...
    public function dataTable (){
        ...
        foreach ($output_arr['data'] as $key => $value) {
            ...
            $output_arr['data'][$key][count($output_arr['data'][$key])  - 1] .= '<a id="btnAddFile" href="'.base_url().'pendidikan/file/'.$id.'" type="button" data-src="'.$id.'" title="File"><i class="fa fa-file" data-id=""></i></a>';
            $output_arr['data'][$key][count($output_arr['data'][$key])  - 1] .= '<a id="btnEditRow" data-toggle="modal" data-target="#nameModal_pendidikan" class="modalButtonPendidikan mClass"  href="javascript:;" type="button" data-src="'.$id.'" title="Edit"><i class="fa fa-pencil" data-id=""></i></a>';
            
            $output_arr['data'][$key][count($output_arr['data'][$key])  - 1] .= '<a style="cursor:pointer;" data-toggle="modal" class="mClass" onclick="setId('.$id.', \'pendidikan\')" data-target="#cnfrm_delete" title="delete"><i class="fa fa-trash-o" ></i></a>';
  ...
  /**
     ...
     * @return Void
     */
    public function modal_pendidikan() {
        if($this->input->post('id')){
            $data['pendidikanData'] = getDataByid('pendidikan',$this->input->post('id'),'no'); 
            echo $this->load->view('add_pendidikan', $data, true);
        } else {
            echo $this->load->view('add_pendidikan', '', true);
        }
        exit;
    }
        ...
        if(!empty($id)){
            $data['data']= $this->pendidikan_model->get_pendidikan($id);
            $this->load->view('add_pendidikan', $data);  
        } else {
            $this->load->view('add_pendidikan'); 
        }
    }
    ...
    public function add_edit($id='') {
        ...
        if($id = $this->input->post('id')) {
			...
			$update['rekomendasi']=$data['rekomendasi'];
			 ...
			 $this->pendidikan_model->updateRow('pendidikan', 'no', $id, $update);	

            $this->session->set_flashdata('messagePr', 'Your data updated Successfully..');
            redirect(base_url().'pendidikan/', 'refresh'); 
        } else { 
            $userid = $this->session->userdata ('user_details')[0]->users_id;
			...
			$id=$this->pendidikan_model->insertRow('pendidikan',$insert);
				$ins=0;
				if($this->upload->do_upload('bebankerja_bukti')){
					$ins++;
					$files['id_modul']=$id;
			...
			if($ins>0)$this->pendidikan_model->updateRow('pendidikan', 'no', $id, $update);	
       ...
       redirect(base_url().'pendidikan/index', 'refresh');
    }
	...
	public function fileTable($param1='',$param2 =''){
        ...
        foreach ($output_arr['data'] as $key => $value) {
			...
			$output_arr['data'][$key][count($output_arr['data'][$key])  - 1] .= '<a style="cursor:pointer;" data-toggle="modal" class="mClass" onclick="delFile('.$id.',\'pendidikan\')" data-target="#cnfrm_delete" title="delete"><button type="button" class="btn-sm  btn btn-danger modalButtonPendidikan" ><i class="fa fa-trash-o" ></i>  Delete File</button></a>';
	...
	public function file($param1 ='') {
            ...
            $this->load->view('filependidikan_table',$page_parameter);                
            $this->load->view('include/footer');   
        } 
	...
	public function addfile($param1 ='',$param2='') {
			if($param1=='upload' && $param2 !=''){
				...
				redirect(base_url() . 'pendidikan/file/'.$param2, 'refresh');
				}else{
