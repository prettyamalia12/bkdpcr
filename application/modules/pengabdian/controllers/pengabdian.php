...
class pengabdian extends CI_Controller {
    ...
    function __construct() {
        ...
        $this->pengabdian_id = isset($this->session->get_pengabdiandata()['pengabdian_details'][0]->id)?$this->session->get_pengabdiandata()['pengabdian_details'][0]->no:'1';
    ...
    public function index() {
            ...
            $this->load->view('pengabdian_table');                
    ...
    public function dataTable (){
        ...
        foreach ($output_arr['data'] as $key => $value) {
            ...
            $output_arr['data'][$key][count($output_arr['data'][$key])  - 1] .= '<a id="btnAddFile" href="'.base_url().'pengabdian/file/'.$id.'" type="button" data-src="'.$id.'" title="File"><i class="fa fa-file" data-id=""></i></a>';
            $output_arr['data'][$key][count($output_arr['data'][$key])  - 1] .= '<a id="btnEditRow" data-toggle="modal" data-target="#nameModal_pengabdian" class="modalButtonPengabdian mClass"  href="javascript:;" type="button" data-src="'.$id.'" title="Edit"><i class="fa fa-pencil" data-id=""></i></a>';
            
            
            $output_arr['data'][$key][count($output_arr['data'][$key])  - 1] .= '<a style="cursor:pointer;" data-toggle="modal" class="mClass" onclick="setId('.$id.', \'pengabdian\')" data-target="#cnfrm_delete" title="delete"><i class="fa fa-trash-o" ></i></a>';
  ...
  /**
    ...
    public function get_modal() {
        ...
        if($this->input->post('id')){
            $data['pengabdianData'] = getDataByid('pengabdian',$this->input->post('id'),'no'); 
            echo $this->load->view('add_pengabdian', $data, true);
        } else {
            echo $this->load->view('add_pengabdian', '', true);
        }
        exit;
    }
    ...
    public function modal_pengabdian() {
        //is_login();
        if($this->input->post('id')){
            $data['pengabdianData'] = getDataByid('pengabdian',$this->input->post('id'),'no'); 
            echo $this->load->view('add_pengabdian', $data, true);
	...
	public function add_edit($id='') {
        ...
        if($id = $this->input->post('id')) {
			...
			$update['rekomendasi']=$data['rekomendasi'];
			 ...
			 $this->pengabdian_model->updateRow('pengabdian', 'no', $id, $update);	

            $this->session->set_flashdata('messagePr', 'Your data updated Successfully..');
            redirect(base_url().'pengabdian/', 'refresh'); 
        } else { 
            $userid = $this->session->userdata ('user_details')[0]->users_id;
			...
			$id=$this->pengabdian_model->insertRow('pengabdian',$insert);
				$ins=0;
				if($this->upload->do_upload('bebankerja_bukti')){
					$ins++;
					$files['id_modul']=$id;
			 ...
			 if($ins>0)$this->pengabdian_model->updateRow('pengabdian', 'no', $id, $update);	
        ...
        $ids = explode('-', $id);
        foreach ($ids as $id) {
            $this->pengabdian_model->delete($id); 
        }
       redirect(base_url().'pengabdian', 'refresh');
    }
	...
	public function deletefile($id){
       ...
       redirect(base_url().'pengabdian/index', 'refresh');
    }
	...
	public function fileTable($param1='',$param2 =''){
        ...
        foreach ($output_arr['data'] as $key => $value) {
			...
			$output_arr['data'][$key][count($output_arr['data'][$key])  - 1] .= '<a style="cursor:pointer;" data-toggle="modal" class="mClass" onclick="delFile('.$id.',\'pengabdian\')" data-target="#cnfrm_delete" title="delete"><button type="button" class="btn-sm  btn btn-danger modalButtonPengabdian" ><i class="fa fa-trash-o" ></i>  Delete File</button></a>';
            $output_arr['data'][$key][0] = '<input type="checkbox" name="selData" value="'.$output_arr['data'][$key][0].'">';
	...
	public function file($param1 ='') {
            ...
            $this->load->view('filepengabdian_table',$page_parameter);                
            $this->load->view('include/footer');   
        } 
	...
	public function addfile($param1 ='',$param2='') {
			if($param1=='upload' && $param2 !=''){
				...
				redirect(base_url() . 'pengabdian/file/'.$param2, 'refresh');
				}else{
