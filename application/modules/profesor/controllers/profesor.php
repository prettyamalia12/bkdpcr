...
class profesor extends CI_Controller {
    ...
    function __construct() {
        ...
        $this->profesor_id = isset($this->session->get_profesordata()['profesor_details'][0]->id)?$this->session->get_profesordata()['profesor_details'][0]->no:'1';
    ...
    public function dataTable (){
        ...
        foreach ($output_arr['data'] as $key => $value) {
            ...
            $output_arr['data'][$key][count($output_arr['data'][$key])  - 1] .= '<a id="btnAddFile" href="'.base_url().'profesor/file/'.$id.'" type="button" data-src="'.$id.'" title="File"><i class="fa fa-file" data-id=""></i></a>';
			$output_arr['data'][$key][count($output_arr['data'][$key])  - 1] .= '<a id="btnEditRow" data-toggle="modal" data-target="#nameModal_profesor" class="modalButtonProfesor mClass"  href="javascript:;" type="button" data-src="'.$id.'" title="Edit"><i class="fa fa-pencil" data-id=""></i></a>';
            
            
            $output_arr['data'][$key][count($output_arr['data'][$key])  - 1] .= '<a style="cursor:pointer;" data-toggle="modal" class="mClass" onclick="setId('.$id.', \'profesor\')" data-target="#cnfrm_delete" title="delete"><i class="fa fa-trash-o" ></i></a>';
  ...
  /**
    ...
    public function get_modal() {
        ...
        if($this->input->post('id')){
            $data['profesorData'] = getDataByid('profesor',$this->input->post('id'),'no'); 
            echo $this->load->view('add_profesor', $data, true);
        } else {
            echo $this->load->view('add_profesor', '', true);
        }
        exit;
    }
    ...
    public function modal_profesor() {
        //is_login();
        if($this->input->post('id')){
            $data['profesorData'] = getDataByid('profesor',$this->input->post('id'),'no'); 
            echo $this->load->view('add_profesor', $data, true);
 ...
 public function add_edit($id='') {
        ...
        if($id = $this->input->post('id')) {
			...
			$update['tahun']=$data['tahun'];
			 ...
			 $this->profesor_model->updateRow('profesor', 'no', $id, $update);	
			...
			$id=$this->profesor_model->insertRow('profesor',$insert);
				$ins=0;
				if($this->upload->do_upload('bebankerja_bukti')){
					$ins++;
					$files['id_modul']=$id;
			 ...
			 if($ins>0)$this->profesor_model->updateRow('profesor', 'no', $id, $update);	
	...
	public function deletefile($id){
       ...
       redirect(base_url().'profesor/index', 'refresh');
    }
	...
	public function fileTable($param1='',$param2 =''){
        ...
        foreach ($output_arr['data'] as $key => $value) {
			...
			$output_arr['data'][$key][count($output_arr['data'][$key])  - 1] .= '<a style="cursor:pointer;" data-toggle="modal" class="mClass" onclick="delFile('.$id.',\'profesor\')" data-target="#cnfrm_delete" title="delete"><button type="button" class="btn-sm  btn btn-danger modalButtonProfesor" ><i class="fa fa-trash-o" ></i>  Delete File</button></a>';
            $output_arr['data'][$key][0] = '<input type="checkbox" name="selData" value="'.$output_arr['data'][$key][0].'">';
			}
	...
	public function file($param1 ='') {
            ...
            $this->load->view('fileprofesor_table',$page_parameter);                
            $this->load->view('include/footer');   
        } 
	...
	public function addfile($param1 ='',$param2='') {
			if($param1=='upload' && $param2 !=''){
				...
				redirect(base_url() . 'profesor/file/'.$param2, 'refresh');
				}else{
